...
export function registerExtractHandlers(ctx: AppContext) {
  ...
  async function extractor(arg: ExtractArg){
    try {
      ctx.gb = {}
      let file
  ...
  ipcMain.on('updateVersion', async (ev, arg) => {
    ...
    try {
      ...
      for(let i in (fileList1)){
        ...
        ctx.mainWindow!.webContents.send('loading', Number(i)/fileList1.length*100);
        await sleep(0)
      }
      ctx.mainWindow!.webContents.send('alert', '완료되었습니다')
      endThis()
    } catch (err) {
