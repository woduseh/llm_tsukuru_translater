    ...
    if (origType === 'string') {
        if (orig !== trans) {
            if (stringPolicy === 'deny') {
                ...
                checkControlChars(orig, trans, path, issues);
                checkTextShift(orig, trans, path, issues);
            }
        }
        return issues;
    }
...
function isTextShifted(origStr, transStr) {
    if (origStr === transStr)
        return false;
    const origTrimmed = origStr.trim();
    // 빈 문자열이 채워진 경우
    if (origStr === '' && transStr !== '')
        return true;
    // 플레이스홀더가 덮어씌워진 경우
    if (PLACEHOLDER_RE.test(origTrimmed) && !PLACEHOLDER_RE.test(transStr.trim()))
        return true;
    // 대사 자리에 마커가 밀려 들어온 경우
    if (!PLACEHOLDER_RE.test(origTrimmed) && PLACEHOLDER_RE.test(transStr.trim()))
        return true;
    // 이름표 괄호가 사라진 경우
    if (NAME_BRACKET_FULL_RE.test(origTrimmed) && !NAME_BRACKET_PARTIAL_RE.test(transStr))
        return true;
    // 기호 전용 줄에 문자가 들어온 경우
    if (origTrimmed.length > 0 && /^[.…!！?？、。·\-~～♪♫★☆♥♡●○■□▲△▼▽\s]+$/.test(origTrimmed)) {
        if (/[a-zA-Z\u3040-\u9FFF\uAC00-\uD7AF]/.test(transStr) && transStr.trim() !== origTrimmed)
            return true;
    }
    return false;
}
        ...
        for (const key of Object.keys(origObj)) {
            if (key in transObj) {
                ...
                if (COMMENT_KEY_RE.test(key) && typeof origObj[key] === 'string' && typeof transObj[key] === 'string') {
                    result[key] = isTextShifted(origObj[key], transObj[key])
